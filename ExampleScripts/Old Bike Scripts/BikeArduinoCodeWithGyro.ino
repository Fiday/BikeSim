...
*/
#include <Uduino.h>
#include <ArduinoBLE.h>  //Version 1.2.1
#include <Wire.h>
...
void setup() {
  ...
  uduino.print(F("MPU6050 status: "));
  uduino.println(status);

  uduino.println(F("Calculating offsets, do not move MPU6050"));
  delay(1000);
  ...
  while (peripheral.connected()) {
    ...
    if ((millis() - lastMillisIDB) >= 95) {
      ...
      for (int i = 0; i < descriptorValueSize; i++) {
        ...
        CombinedBrakeForce = CombinedBrakeForce;
      }
    }
    ...
    if ((millis() - lastMillisResistance) >= 415) {
      ...
      Resistance = map(CombinedBrakeForce, 0, 200, 0, 10);  //Resistance angle
      uint8_t OpCode = 0x04;
      ...
      uint8_t PotResistance[3] = { OpCode, 0, ResistanceSupport };  //OpCode, LSP, MSB
